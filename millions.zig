...
const points_per_host: usize = 100_000;
...
fn runBenchmark(allocator: std.mem.Allocator) !void {
    ...
    std.debug.print("\n", .{});
    std.debug.print("ingested {d} (1M) points in {d:.2}s\n", .{ items_written, elapsed_secs });
    ...
    std.debug.print("  Total: {d} bytes ({d:.2} MB)\n", .{ total_size, @as(f64, @floatFromInt(total_size)) / 1024.0 / 1024.0 });
    ...
    const lower_bound: i64 = host_end - 100_000;
    const upper_bound: i64 = host_end - 1;

    std.debug.print("  Querying {s} range {d}-{d} (100K points)\n", .{ query_host, lower_bound, upper_bound });
