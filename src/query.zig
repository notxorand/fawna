...
/// Query operation state for aggregation during polling
...
/// Result of parsing series and tags from CSV format
pub const SeriesAndTags = struct {
    series: []const u8,
    tags: []const []const u8,
};

/// Result of aggregating query across multiple series
...
pub const Query = struct {
    ...
    fn parseOp(filter: []const u8) QueryOp {
    ...
    pub fn parseSeriesAndTags(allocator: std.mem.Allocator, csv: []const u8) !SeriesAndTags {
        var series: []const u8 = "temp";
        ...
        while (iter.next()) |raw| {
            ...
            if (std.mem.startsWith(u8, token, "series=") and token.len > "series=".len) {
                series = token["series=".len..];
                continue;
            }

            try tags.append(token);
        }

        return SeriesAndTags{
            .series = series,
            .tags = tags.items,
        };
    }
